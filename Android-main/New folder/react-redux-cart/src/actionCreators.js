...
/******************************************************************************************** */
/**
 * Action Creator para realizar pruebas online con firebase
 */
// const loadProducts = () => {
const loadProductsOnline = () => {
  return dispatch => {
    const productsRef = firebase.database().ref().child('ecommerce');
    // const cartRef = firebase.database().ref().child('cart');
    return productsRef.on('value', snap => {
      const products = snap.val().products;
      const cart = snap.val().cart;
      dispatch({
        type: 'LOAD_PRODUCTS',
        products: products,
        cart:cart
